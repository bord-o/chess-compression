...
pub fn huff_gen(frequencies: Option<HashMap<u16, usize>>) -> (Book<u16>, Tree<u16>) {
    let default_frequencies: HashMap<u16, usize> = HashMap::from([
        (0, 59702781),
        (1, 30721413),
        (2, 19353377),
        (3, 13851683),
    ...
    let freq = match frequencies {
        Some(map) => map,
        None => default_frequencies,
    };

    let (book, tree) = CodeBuilder::from_iter(&freq).finish();
    return (book, tree);
}
